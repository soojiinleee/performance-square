...
class PerformanceManager(models.Manager):
    def get_queryset(self):
        ...
        return super().get_queryset().annotate(
            status=Case(
                When(started_at__lte=today, ended_at__gte=today, then=Value(PerformanceStatus.ONGOING.value)),
...
class Performance(TimeStampedMixin):
    ...
    user = models.ManyToManyField(User, through="PerformanceLike", verbose_name='좋아요 클릭한 유저: m2m')
...
class PerformanceLike(TimeStampedMixin):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="liked_performances")
    performance = models.ForeignKey(Performance, on_delete=models.CASCADE, related_name="liked_users")
